...
row = Xlutils.GetRowCount(path,"Sheet3")
...
for r in range(2,row+1):
    Amount = Xlutils.ReadData(path,"Sheet3",r,1)
    TXN_ID = Xlutils.ReadData(path, "Sheet3", r, 2)
    Merchant_id = Xlutils.ReadData(path, "Sheet3", r, 3)
    Terminal_id = Xlutils.ReadData(path, "Sheet3", r, 4)
    MCC = Xlutils.ReadData(path, "Sheet3", r, 5)
    CARD_NUM = Xlutils.ReadData(path,"Sheet3", r, 6)
    CARD_Exipry = Xlutils.ReadData(path, "Sheet3", r, 7)
    CVV = Xlutils.ReadData(path, "Sheet3", r, 8)
    OTP = Xlutils.ReadData(path, "Sheet3", r, 9)
    ...
    if Message == "Success":
        ...
        Xlutils.WriteData(path,'Sheet3',r,10,"Test case passed")
        driver.get("https://sandbox.isgpay.com/KotakPGRedirect/")
        logger.info("Transaction sccessfull")
    else:
        print("transaction failed")
        Xlutils.WriteData(path, 'Sheet3', r, 10, "Test case failed")
