...
row = Xlutils.GetRowCount(path,"Sheet5")

for r in range(2,row+1):
    MER_TXN_NO= Xlutils.ReadData(path,"Sheet5",r,1)
    AMOUNT=Xlutils.ReadData(path,"Sheet5",r,2)
    PASSCODE=Xlutils.ReadData(path,"Sheet5",r,3)
    TERMINAL_ID=Xlutils.ReadData(path,"Sheet5",r,4)
    MERCHANT_ID=Xlutils.ReadData(path,"Sheet5",r,5)
    TXN_TYPE=Xlutils.ReadData(path,"Sheet5",r,6)
    REF_TXN_NO=Xlutils.ReadData(path,"Sheet5",r,7)
    REFUND_AMOUNT=Xlutils.ReadData(path,"Sheet5",r,8)
    CAPTURE_AMOUNT=Xlutils.ReadData(path,"Sheet5",r,9)
    CURRENCY=Xlutils.ReadData(path,"Sheet5",r,10)
    CANCELLATION_ID=Xlutils.ReadData(path,"Sheet5",r,11)
    REFUND_REASON_CODE=Xlutils.ReadData(path,"Sheet5",r,12)
    ...
    if message == 'Success':
        ...
        Xlutils.WriteData(path,"Sheet5",r,13,"test pass")
        driver.get('https://sandbox.isgpay.com/KotakPGRedirect/merchant/rac/racAPI.jsp')
        logger.info("capture done")
    else:
        print('capture failed')
        Xlutils.WriteData(path, "Sheet5", r, 13, "test fail")
        driver.get('https://sandbox.isgpay.com/KotakPGRedirect/merchant/rac/racAPI.jsp')
